...
//url=$.getJSON('http://http://127.0.0.1:5000/api/v1.0/peizometer-readings').done(function(peizo_data) {
            //console.log(peizo_data)
        //})
...
function buildPlots(selection) {
    staticData.then(function(data) {
        ...
        var hbData = [{
           ...
           var layout = {
            title: '<b>Max and Min Historical PWP Values</b><br>(VWP'+" "+filteredMetadata[0].Node_id+" "+'in kPa)',
            barmode: 'stack',
            xaxis: { title: "Pressure(kPa)"},
            //yaxis: { title: "Max and Min"}
        ...
        scatterChart.html('');
        var trace1 = {
            x: filteredSamples[0].otu_ids, //Dates
            y: filteredSamples[0].sample_values, //Pressure(kPa)
        ...
        var layout ={
            title: " Trend of Water Pressure in VWP "+filteredMetadata[0].Node_id,
            //xaxis: { title: "Date"},
